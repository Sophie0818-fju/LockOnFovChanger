--[[
===============================================================================
LockOnFovChanger v1.1.12
===============================================================================

PURPOSE
-------
LockOnFovChanger adjusts the camera behavior during Lock-On combat in
The Blood of Dawnwalker.

Features:
- Changes the camera FOV while Lock-On is active.
- Preserves the game's normal third-person camera positioning.
- Automatically handles temporary camera mode changes during combat,
  abilities, attacks, and other camera transitions.
- Uses HardLock ON/OFF as the Lock-On lifecycle signal.
- Uses CurrentCombatMode only to detect a transition back to 0 for Combat Exit
  cleanup when a Lock-On session remains active.
- Uses CameraModeStack only to select CameraType while Lock-On remains active;
  it never ends/recoveries the Lock-On session.
- Keeps the camera attached to the lock target during an active Lock-On session and restores the original attachment state on exit.
- Refreshes active camera/FOV state after SetLockTarget target changes.
- Press PageDown to toggle both FOV and camera offset features ON/OFF.

INSTALLATION
------------
1. Install UE4SS and make sure Lua mod support is enabled.
2. Create the following folder:

   ue4ss\Mods\LockOnFovChanger\Scripts\

3. Copy this Lua script into the Scripts folder.
4. Place the configuration file in the same Scripts folder:
...
CONFIGURATION
-------------
Edit LockOnFovChanger.ini to change the following parameters:

FOVEnabled
----------
Enables or disables the Lock-On FOV modification.

true  = Enable Lock-On FOV adjustment
false = Disable Lock-On FOV adjustment
...
LockOnFOV
---------
Sets the FOV used while Lock-On is active.
...
CameraOffsetFix
---------------
Controls the optional camera offset adjustment.

true  = Enable camera offset adjustment
false = Keep the game's normal camera offset
...
When enabled, each game session creates a separate log file with a
timestamp in the filename.

Example:
LockOnFovChanger_v1.1.7_20260909_000000.log
...
clo fovstatus
Display current FOV and Lock-On state.

clo diagstatus
Display current combat/camera diagnostic state.


CAMERA BEHAVIOR
---------------
During Lock-On, CameraModeStack depth is used only for temporary camera behavior:
- depth > Lock-On baseline -> CameraType 1
- depth = Lock-On baseline -> CameraType 0
- depth < Lock-On baseline -> log only; no recovery is performed.

CameraModeStack never starts, ends, or recovers a Lock-On session.
The Lock-On session lifecycle is driven only by HardLock ON/OFF events.

Combat-state handling uses PlayerCombatComponent.CurrentCombatMode only to detect
the end of the combat context. A transition from a non-zero CurrentCombatMode to 0
ends any still-active Lock-On session and restores the saved CameraType/FOV. It does
not use CurrentState for Combat lifecycle decisions.

The optional controller-input observer is disabled by default in v1.0.0 because
the current UE4SS FKey construction path is not reliable on this game build.
It has no effect on Lock-On, camera, FOV, or gameplay.

CameraMode discovery is event-driven for both RebelCameraMode and
/Script/DogwoodCombat.CombatCameraMode, so newly created combat camera objects
can be captured without per-frame object scans. SetLockTarget is also observed
so an active Lock-On can refresh its camera/FOV state after target changes.


AUTHOR / VERSION
----------------
Mod: LockOnFovChanger
Version: 1.1.12
Author: josky
]]
local MOD_NAME = "LockOnFovChanger_v1.1.12"
local SCRIPT_VERSION = "1.1.12"
...
local function build_log_path()
    if script_directory == nil or script_directory == "" then
        return "LockOnFovChanger_v1.1.7_" .. os.date("%Y%m%d_%H%M%S") .. ".log"
    end

    return script_directory .. "\\LockOnFovChanger_v1.1.7_" .. os.date("%Y%m%d_%H%M%S") .. ".log"
end
    ...
    for index = 1, depth do
        ...
        if entry_ok and entry ~= nil then
            ...
            if valid_object(mode) and not is_ability_mode(mode) then
                ...
                if offset ~= nil then
                    local before_y = get_field(offset, "Y")
                    local write_ok = pcall(function()
                        offset.Y = target_y
                    end)
                    local after_y = get_field(offset, "Y")
...
local function refresh_active_lock()
    ...
    if valid_object(camera) then
        ...
        if config.CameraOffsetFix then
            apply_combat_offset_fix(COMBAT_OFFSET_FIX_Y, "LockRefresh")
        end
    end
end
...
apply_lock_fov = function(combat, locked)
    if locked then
        ...
        if config.CameraOffsetFix then
            apply_combat_offset_fix(COMBAT_OFFSET_FIX_Y, "LockOn")
        else
            apply_combat_offset_fix(COMBAT_OFFSET_DEFAULT_Y, "LockOn")
        end
    ...
    if config.CameraOffsetFix then
        apply_combat_offset_fix(COMBAT_OFFSET_DEFAULT_Y, "LockOff")
    end
...
local function handle_new_camera_mode(mode, source)
    ...
    -- Offset is handled separately from this FOV/cache callback.
    -- Keep this discovery callback focused on FOV/cache handling.
...
-- PageDown toggles the overall mod feature state.
-- Both FOV and CameraOffsetFix are switched together.
-- When disabling during Lock-On, restore the original FOV and normal offset.
-- When enabling during Lock-On, immediately reapply the configured FOV/offset.
if type(RegisterKeyBind) == "function" and type(Key) == "table" and Key.PAGE_DOWN ~= nil then
    local ok, err = pcall(function()
        RegisterKeyBind(Key.PAGE_DOWN, function()
            local enable = not (config.FOVEnabled or config.CameraOffsetFix)
            ...
            ExecuteInGameThread(function()
                if enable then
                    ...
                    if lock_active then
                        local fov_ok = write_locked_fov()
                        if not fov_ok then
                            pcall(rebuild_mode_cache)
                            fov_ok = write_locked_fov()
                        end
                        fov_applied = fov_ok
                        apply_combat_offset_fix(COMBAT_OFFSET_FIX_Y, "PageDown")
                    end

                    append_log(
                        "HOTKEY | PageDown | ModEnabled=true" ..
                        " | FOVEnabled=true" ..
                        " | CameraOffsetFix=true"
                    )
                else
                    ...
                    if lock_active then
                        apply_combat_offset_fix(COMBAT_OFFSET_DEFAULT_Y, "PageDown")
                    end
                    ...
                    append_log(
                        "HOTKEY | PageDown | ModEnabled=false" ..
                        " | FOVEnabled=false" ..
                        " | CameraOffsetFix=false"
                    )
                end
    ...
    if ok then
        append_log("HOTKEY REGISTERED | PageDown=Toggle FOV+Offset")
    else
        append_log("HOTKEY REGISTER FAILED | PageDown | Error=" .. tostring(err))
    end
else
    append_log("HOTKEY UNAVAILABLE | PageDown binding API not available")
end
...
RegisterConsoleCommandHandler(
    ...
    function(FullCommand, Parameters, Ar)
        ...
        if command == "?" or command == "" then
            ...
            Ar:Log("PageDown = toggle FOV + Offset")
...
append_log(
    "PageDown toggles FOVEnabled and CameraOffsetFix together. " ..
    "When disabled during Lock-On, FOV and offset are restored to normal values; " ..
    "when enabled during Lock-On, the configured FOV and centered offset are reapplied."
)
